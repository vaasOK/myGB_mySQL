...
DROP DATABASE IF EXISTS vpn_control_db_v3;

CREATE DATABASE vpn_control_db_v3;

USE vpn_control_db_v3;
...
CREATE TABLE `users` (
    u_id         INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'id пользователя'
  , surname      VARCHAR(40)  NOT NULL COMMENT 'фамилия пользователя'
  ...
  , status       ENUM ('active', 'paused', 'blocked', 'deleted') DEFAULT 'paused' COMMENT 'статус пользователя, по умолчанию - "paused"'   -- active | paused | blocked | deleted
...
CREATE TABLE `organizations` (
    org_id      INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'id организации'
  , name        VARCHAR(100) NOT NULL COMMENT 'наименование организации'
  , full_name   VARCHAR(300) COMMENT 'полное наименование организации '
  , main_user   INT UNSIGNED NOT NULL COMMENT 'Контактное лицо'	-- main_contact, users_id 
  , description VARCHAR(200) COMMENT 'комментарий, примечание'
...
CREATE TABLE `user_categorys` (
    u_cat_id   INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'id категории пользователей'
  , name ENUM ('normal', 'VIP', 'security', 'sys_admin') COMMENT 'имя категории пользователей'     -- normal | VIP | security | org_admin | sys_admin
...
CREATE TABLE `users_profiles` (
    user_id           INT UNSIGNED NOT NULL UNIQUE PRIMARY KEY COMMENT 'id пользователя, чей профиль'
  , changer_id        INT UNSIGNED NOT NULL COMMENT 'id пользователя, кто изменял user/профиль'
  , categorys_id      INT UNSIGNED NOT NULL COMMENT 'id категории  пользователя (роль?)'
  , organizations_id  INT UNSIGNED NOT NULL COMMENT 'id организации, от какой организации user'
  , clnt_params_grps_id INT UNSIGNED COMMENT 'id группы параметров'
  , description       VARCHAR(200) COMMENT 'комментарий, примечание'
...
CREATE TABLE `org_contacts` (
    ...
    organization_id INT UNSIGNED NOT NULL COMMENT 'id организации'
...
) COMMENT 'Дополнительные контактные лица организации';
...
CREATE TABLE `emails` (
    em_id         INT UNSIGNED NOT NULL UNIQUE PRIMARY KEY COMMENT 'id электронного адреса'
  , email_address VARCHAR(120) NOT NULL UNIQUE COMMENT 'электронный адрес, e-mail'
  , user_id       INT UNSIGNED NOT NULL COMMENT 'id пользователя'
  , description   VARCHAR(200) COMMENT 'комментарий, примечание'
...
CREATE TABLE `phones` (
    ph_id       INT UNSIGNED NOT NULL UNIQUE PRIMARY KEY COMMENT 'id номера Телефона'
  , phone       BIGINT unsigned NOT NULL UNIQUE COMMENT 'номер Телефона'
  , user_id     INT UNSIGNED NOT NULL COMMENT 'id пользователя'

  , description VARCHAR(200) COMMENT 'примечание'
...
CREATE TABLE `vpn_keys` (
  ...
  , key_dest   BIT         -- 0 user | 1 server
    -- Длина поля рассчитана на размещене в нём ключа в явном виде, но для создания БД пишем сюда имена файлов с ключами
...
CREATE TABLE `key_bindings` (
    key_id   INT UNSIGNED NOT NULL UNIQUE COMMENT 'id ключа'            -- uniq
  , user_id  INT UNSIGNED NOT NULL COMMENT 'id пользователя'
...
CREATE TABLE `srv_groups` (
    srv_gr_id         INT UNSIGNED NOT NULL UNIQUE PRIMARY KEY COMMENT 'id группы серверов'
  , name              VARCHAR(40)  NOT NULL UNIQUE COMMENT 'имя группы серверов'
...
CREATE TABLE `servers` (
  ...
  , srv_group_id INT UNSIGNED      NOT NULL COMMENT 'id группы серверов'
...
DROP TABLE IF EXISTS `clnt_params_grps`;

CREATE TABLE `clnt_params_grps` (
    cl_par_id         INT UNSIGNED NOT NULL UNIQUE PRIMARY KEY COMMENT 'id группы параметров'
  , name              VARCHAR(40)  NOT NULL UNIQUE COMMENT 'имя группы параметров'
  , params_files_path VARCHAR(4096) COMMENT 'путь на файловой системе, где размещаются файлы параметров'
  , params_file       VARCHAR(256)  COMMENT 'имя файла параметров'
  , description       VARCHAR(200)  COMMENT 'комментарий, примечание'
...
ALTER TABLE vpn_control_db_v3.users_profiles
ADD CONSTRAINT clnt_params_fk
FOREIGN KEY (clnt_params_grps_id) REFERENCES vpn_control_db_v3.clnt_params_grps(cl_par_id)
;

ALTER TABLE vpn_control_db_v3.srv_groups
...
INSERT INTO vpn_control_db_v3.clnt_params_grps
...
INSERT INTO vpn_control_db_v3.users 
...
INSERT INTO vpn_control_db_v3.organizations
...
INSERT INTO vpn_control_db_v3.user_categorys 
 (u_cat_id, name)
VALUES
 (1, 'sys_admin')
,(2, 'security')
...
INSERT INTO vpn_control_db_v3.users_profiles
 (user_id , changer_id , categorys_id , organizations_id , clnt_params_grps_id , description) 
VALUES
...
INSERT INTO vpn_control_db_v3.org_contacts
...
INSERT INTO vpn_control_db_v3.emails
...
INSERT INTO vpn_control_db_v3.phones
...
INSERT INTO vpn_control_db_v3.PKIs
...
INSERT INTO vpn_control_db_v3.srv_groups
 (srv_gr_id, name, clnt_params_grps_id, description)
VALUES
...
INSERT INTO vpn_control_db_v3.vpn_keys
...
INSERT INTO vpn_control_db_v3.key_bindings
 (key_id, user_id)
VALUES
 ( 10, 1)
,( 11, 2)
...
INSERT INTO vpn_control_db_v3.servers
